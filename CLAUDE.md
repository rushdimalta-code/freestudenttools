...
- **`scholarships.html`'s remaining LCP gap** is client-side-rendering of a 246-item list, root-caused above — a pre-existing, already-documented architectural characteristic (not introduced this session), and fixing it properly means pre-rendering/pagination, a separate scoped project, not a same-session patch.
